...
### 7.1 静态图标(manifest `Icon` / 插件 / 分类)

上面讲的是运行时按键面。app 内 action 列表的**预览图标**是另一条链路:插件只提供 `assets/icons/` 下的静态 `.svg`,自身不栅格化,由宿主(Electron)渲染。插件唯一能控的清晰度杠杆是 SVG 根元素声明的 `width`/`height`;宿主疑似复用与按键面同一栅格化器(按声明尺寸先渲位图再缩放),声明过小会在 HiDPI(如 3840×2160 面板跑 2560×1440,1.5× 缩放)下发糊。

约定:`assets/icons/` 下全部图标(6 个 action + `pluginIcon` + `categoryIcon`)统一声明 `width/height="512"`,`viewBox` 各自保持不变(矢量放大零画质/体积代价)。此前 bambustatus/speedtest 声明 144、其余 256,尺寸不一致既是糊的疑因,也是排查干扰项;统一后消除该变量。若抬到 512 后全组仍一致发糊,则落到宿主未乘 `devicePixelRatio` 的 HiDPI bug,插件侧无法修,需走宿主工单。
