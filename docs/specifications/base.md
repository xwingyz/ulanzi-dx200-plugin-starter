...
| 宿主事件 | 基座行为 |
| --- | --- |
| `onAdd` / `onParamFromApp` | 以本地持久化设置为权威合并，归一化、必要时回写存储、渲染、调用 `onReady`，并把权威设置回推 Inspector |
...
公共主题：`mint`、`ember`、`mono`、`signal`、`neon`、`ice`、`sunset`、`forest`、`sand`。每个主题提供 `accent`、`canvas`、`panel`、`shell`、`text`、`muted`、`low`、`contrast`。颜色只能由 theme 这一条轴驱动。
