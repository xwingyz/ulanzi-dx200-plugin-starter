...
- 非打印状态不再另画第三行小字详情。稳定且不需要用户处理的 `IDLE`、`CONNECTING`、`FINISHED` 只显示“空闲”“连接中”“已完成”，禁止拼接等待提示、读取提示、任务名、模型名或文件名。`PREPARING`、`PAUSED` 有细分阶段时都只显示子状态，不再添加“准备中”或“已暂停”前缀；没有细分阶段时才显示对应主状态，避免“已暂停 · 暂停：耗材用尽”这类重复语义。`CONFIG_REQUIRED`、`FAILED`、`INCOMPATIBLE` 只保留单个可行动主状态，不再拼接说明句。
- 状态文字以标题分隔线的 170 个设计单位为可用宽度，左右各保留至少 3 个设计单位，实际文字预算不超过 164。字号必须根据字形视觉宽度在 18–34px 内自适应，不能只按字符数判断。仓库内置的全部中英文状态和 21 个细分阶段不得出现省略号；只有打印机返回的未知原始子状态可以按视觉宽度截断并显示省略号。
- 离线隐藏旧进度，状态行使用“离线 · `<紧凑更新时间>`”，更新时间最多保留两位数并使用紧凑单位，例如 `Offline · 59m`、`离线 · 59 分`；没有成功更新时间时只显示“离线”。
...
- 专项测试覆盖配置发现、SSDP 解析、增量合并、状态映射、时间计算、完成锁存、3 分钟自动过期、单击刷新与全显示态渲染；全显示态必须枚举 21 个细分阶段和中英文两种语言，按 164 个设计单位的视觉宽度预算检查字号与截断。
- 根目录 `npm test` 全绿。
