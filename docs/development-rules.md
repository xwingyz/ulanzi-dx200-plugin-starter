...
**看插件运行日志要开宿主的「调试模式」**。`log()` 只写 `console.log`，走插件进程的 stdout；
Studio 是 GUI 应用、没有终端接管，默认情况下这一路直接被丢弃，这也是过去"插件日志看不到"的原因。
宿主自带解法：插件管理界面左下角勾选「调试模式」，重启 Studio 后，插件进程的 stdout 会被原样捕获到
`~/Library/Application Support/Ulanzi/UlanziDeck/logs/<插件UUID>/<插件UUID>_<PID>.log`——
纯文本、按插件分文件，不用解码 xlog（宿主自己的 `UlanziDeck_YYYYMMDD.xlog` 是 mars 二进制格式，另说）。
注意三点：勾选只对**重启后新拉起**的插件进程生效；文件按 PID 命名，每次重启都新增一个，宿主不做清理，
需要偶尔手动清；调试模式默认关闭，排查问题前先确认它是开的。
高频、结构化、要长期留存的数据不要走 `log()`，走 `appendDiagnosticLog`。
