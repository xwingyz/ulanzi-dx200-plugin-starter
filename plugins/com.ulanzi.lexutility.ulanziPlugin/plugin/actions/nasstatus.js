...
export function createNasStatusAction(runtime) {
  ...
  async function runProbe(instance, probeSettings = {}, options = {}) {
    ...
    if (!isCompleteSettings(merged)) {
      payload = { status: 'incomplete', message: 'Enter the address, username, and password first.' };
    } else {
      ...
      if (result.state === 'ONLINE') {
        payload = {
          ...
          message: `Connected to ${result.system.hostname || 'NAS'}${result.system.model ? ` (${result.system.model})` : ''}.`,
        ...
        payload = { status: 'offline', message: 'Cannot reach the NAS. Check the address, port, and network.' };
      } else {
        payload = { status: 'error', kind: result.kind, message: `${ERROR_LABELS[result.kind] || 'API error'}. Check account permissions and DSM settings.` };
      }
    }
  ...
  function renderStorageRow(y, height, volume, theme, language) {
    ...
    if (!volume) {
      return `
        <g data-row="storage">
          ${rowPanel(y, height, theme)}
          <text x="128" y="${(centerY + 6).toFixed(1)}" text-anchor="middle" fill="${theme.muted}" font-size="17">${escapeXml(t('No available volume', language))}</text>
        </g>`;
    }
  ...
  function renderNasStatus(instance) {
    ...
    if (state === 'CONFIG_REQUIRED') {
      body = `<text x="128" y="150" text-anchor="middle" fill="${theme.text}" font-size="34" font-weight="750">${escapeXml(t('Setup required', language))}</text>`
        + `<text x="128" y="184" text-anchor="middle" fill="${theme.muted}" font-size="17">${escapeXml(t('Open the Inspector to enter connection details', language))}</text>`;
    } else if (state === 'PENDING') {
      body = `<text x="128" y="150" text-anchor="middle" fill="${theme.text}" font-size="34" font-weight="750">${escapeXml(t('Connecting', language))}</text>`
        + `<text x="128" y="184" text-anchor="middle" fill="${theme.muted}" font-size="17">${escapeXml(t('Reading NAS status', language))}</text>`;
    } else if (state === 'OFFLINE') {
      body = `<text x="128" y="152" text-anchor="middle" fill="${theme.muted}" font-size="38" font-weight="800">${escapeXml(t('Offline', language))}</text>`
        + `<text x="128" y="190" text-anchor="middle" fill="${theme.low}" font-size="17">${escapeXml(t('Last online', language))} ${escapeXml(formatAge(instance.lastSeenAt, Date.now(), (key) => t(key, language)))}</text>`;
    } else if (state === 'ERROR') {
      body = `<text x="128" y="150" text-anchor="middle" fill="${theme.crit}" font-size="36" font-weight="800">${escapeXml(t('Error', language))}</text>`
        + `<text x="128" y="186" text-anchor="middle" fill="${theme.muted}" font-size="18" font-weight="600">${escapeXml(t(ERROR_LABELS[instance.errorKind] || 'API error', language))}</text>`;
