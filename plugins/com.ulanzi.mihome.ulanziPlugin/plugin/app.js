...
const PLUGIN_UUID = 'com.ulanzi.ulanzistudio.mihome';
...
// Home Assistant 连接信息是插件级共享配置，不是某个 action 的私有设置：本插件下所有
// action 共用同一个 HA 实例，令牌改一次就该全部生效，不该散落进每个 action 的
// property inspector。这个文件不进 git（见 .gitignore），只在本机手填一次。
// 覆盖路径同样只给测试用，宿主永远不设它。
const HA_CONFIG_PATH = process.env.ULANZI_MIHOME_CONFIG_PATH
  ? path.resolve(process.env.ULANZI_MIHOME_CONFIG_PATH)
  : path.join(__dirname, '..', 'config', 'local.json');
const HA_REQUEST_TIMEOUT_MS = 8_000;
...
// ---- Home Assistant 共享客户端：插件级配置，注入给所有 action（见 development-rules.md
// §「共享原语」——第二个 action 要用同一能力时提升到这里，不在 action 里各自实现）。----

function loadHaConfig(options = {}) {
  const fsImpl = options.fsImpl ?? fs;
  const configPath = options.configPath ?? HA_CONFIG_PATH;
  try {
    const raw = JSON.parse(fsImpl.readFileSync(configPath, 'utf8'));
    const baseUrl = typeof raw?.haBaseUrl === 'string' ? raw.haBaseUrl.trim().replace(/\/+$/, '') : '';
    const token = typeof raw?.haToken === 'string' ? raw.haToken.trim() : '';
    return { baseUrl, token };
  } catch (error) {
    if (error?.code !== 'ENOENT') {
      log('mihome config read failed', error?.message || error);
    }
    return { baseUrl: '', token: '' };
  }
}

// 用 node:http(s) 而不是 fetch：与 nasstatus.js 的取舍一致（局域网自签证书场景需要
// rejectUnauthorized 可控），并保持仓库内网络 I/O 写法统一。
function haHttpJson(method, url, token, body, options = {}) {
      ...
      request = lib.request(parsed, {
        ...
        response.on('end', () => {
          ...
          if (status === 401 || status === 403) {
            finish({ ok: false, kind: 'AUTH', status });
            return;
          }
          if (status < 200 || status >= 300) {
            finish({ ok: false, kind: 'HTTP', status });
            return;
          }
          const text = Buffer.concat(chunks).toString('utf8');
          if (!text) {
            finish({ ok: true, json: null });
            return;
          }
          try {
            finish({ ok: true, json: JSON.parse(text) });
          } catch {
...
function createHaClient(options = {}) {
  const config = options.config ?? loadHaConfig(options);
  const request = options.request ?? haHttpJson;
  const configured = Boolean(config.baseUrl && config.token);
  return {
    configured,
    getState(entityId) {
      if (!configured) return Promise.resolve({ ok: false, kind: 'CONFIG' });
      return request('GET', `${config.baseUrl}/api/states/${encodeURIComponent(entityId)}`, config.token);
    },
    callService(domain, service, entityId) {
      if (!configured) return Promise.resolve({ ok: false, kind: 'CONFIG' });
      return request(
        'POST',
        `${config.baseUrl}/api/services/${encodeURIComponent(domain)}/${encodeURIComponent(service)}`,
        config.token,
        { entity_id: entityId },
      );
    },
...
// 同时往新旧两个键位推图，被拖过的键面在两份状态之间跳变。真机实测过后果：一个采样类按键
// 被拖两次后有三个实例并行落盘，联网类 action 对同一台设备挂着两条长连接。
...
// 供用量/进度类 action 共用，保证并排摆放时行高、字号与填充观感必然一致。
//
...
function renderMeterRow(geometry, theme, options = {}) {
  ...
  return `
      ...
      <text x="${x + width * 0.56 + centerTextXOffset}" y="${textY}"${verticalAlign} text-anchor="end" fill="${theme.text}" font-weight="800" font-family="${font}">${numeric(value, fontSize * 1.26, UNIT_FONT_SIZE)}</text>
...
// render 会重复执行业务。实体按压视觉由宿主负责，插件在 keydown 不提交图标。
