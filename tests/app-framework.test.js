...
test('Lex Utility only exposes production actions', () => {
  ...
  assert.deepEqual(Object.keys(lexActionConfigs).sort(), ['bambustatus', 'chatgptusage', 'claudeusage', 'deepseekusage', 'healthbreak', 'latency', 'nasstatus', 'pomowave', 'speedtest', 'systemstatus']);
  assert.deepEqual(
    manifest.Actions.map((action) => action.UUID.split('.').at(-1)).sort(),
    ['bambustatus', 'chatgptusage', 'claudeusage', 'deepseekusage', 'healthbreak', 'latency', 'nasstatus', 'pomowave', 'speedtest', 'systemstatus'],
  );
});
